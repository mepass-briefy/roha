...
features는 외부 사실 검색이 아니라 요구를 펼치는 역할이다. 모든 확장 기능을 category로 태깅한다.

1. Explicit Requirement: 사용자가 직접 명시. provenance=fact. source=requirement:/ux:.
2. Derived Feature: 요구에서 직접 추론 가능한 기능. provenance=inference. source=derived:<근거>.
3. Operational Assumption: 일반 운영 관행 기반 추론. provenance=inference(operational). source=derived:operational:<근거>.
4. Business Decision: 사업 판단이 필요. 자동 채택 금지. features에 넣지 말고 open_questions로 전환(provenance=open_question).

경계 규칙: Derived/Operational/Business 판단이 애매하면 더 보수적인 쪽(Business=Open Question)으로. 발명보다 질문.
Fabrication 차단: 어느 요구에서도 도출되지 않는 신규 기능은 출력하지 않는다.

web_search 옵션(FEATURES_SEARCH=on): strategy가 찾은 경쟁사의 기능만 조사한다(경쟁사 재발굴 금지). 발견 기능은 Competitive Reference(provenance=fact, 출처 URL). "경쟁사에 있으니 우리도"는 자동 채택 금지 -> Business Decision으로 open_questions에 남긴다. 검색에 없는 경쟁사 기능을 지어내지 않는다.
